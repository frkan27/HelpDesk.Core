...
namespace HelpDesk.BLL.Repository.Fault_Repo
{
    public class Fault_Repo : RepoBase<FaultRecord, int>
    {
        ...
        public Fault_Repo(MyContext dbContext, MembershipTools membershipTools, HttpContextAccessor httpContextAccessor) : base(dbContext)
        {
        ...
        public async Task<BussinesLayerResult<FaultRecord>> FaultAdd(FaultRecord data)
        {
            ...
            {
                res.Result.CustomerId = userId;
                var dbresult = Insert(data);
                if (dbresult > 0)
                {
                    res.AddError(ErrorMessageCode.FaultRecordNotAdd, "Sistemde bir hata oluştu..");
                }
